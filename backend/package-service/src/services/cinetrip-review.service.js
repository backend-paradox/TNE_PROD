...
class CineTripReviewService {
  ...
  async getReviewsByPackageId(packageId) {
    try {
      ...
      const pkg = await prisma.cineTripPackage.findFirst({
        where: {
          OR: [
            { id: packageId },
            { slug: packageId },
  ...
  async createReview(packageId, reviewData) {
    try {
      ...
      const existingReview = await prisma.cineTripReview.findFirst({
        where: {
          packageId: pkg.id,
      ...
      // Create the review
      const review = await prisma.cineTripReview.create({
        data: {
          packageId: pkg.id,
          userId: reviewData.userId,
  ...
  async markReviewHelpful(reviewId) {
    try {
      const review = await prisma.cineTripReview.update({
        where: { id: reviewId },
        data: {
          helpful: {
            increment: 1
...
module.exports = new CineTripReviewService();
