...
// Logger helper
const log = {
  info: (msg, meta = {}) => console.log(`[UPLOAD-SERVICE] INFO: ${msg}`, JSON.stringify(meta)),
  warn: (msg, meta = {}) => console.warn(`[UPLOAD-SERVICE] WARN: ${msg}`, JSON.stringify(meta)),
  error: (msg, meta = {}) => console.error(`[UPLOAD-SERVICE] ERROR: ${msg}`, JSON.stringify(meta)),
};
...
const LOCAL_BASE_URL = process.env.LOCAL_UPLOAD_URL || 'http://localhost:3002/uploads/avatars';
...
class UploadService {
  ...
  generateFileName(userId, originalName) {
    const ext = 'webp'; // Always save as WebP after processing
  ...
  async uploadToS3(buffer, fileName, contentType = 'image/webp') {
    ...
    log.info('File uploaded to S3', { key, bucket: S3_BUCKET });
  ...
  async uploadToLocal(buffer, fileName) {
    ...
    log.info('File saved locally', { path: filePath });
   ...
   */
  async uploadAvatar(file, userId, existingUrl = null) {
    // Validate file
    this.validateFile(file);

    // Process image
    const processedBuffer = await this.processImage(file.buffer, 'large');
    ...
    if (useS3 && this.s3Client) {
      ...
      if (existingUrl && existingUrl.includes('s3.')) {
        await this.deleteFromS3(existingUrl);
      }
    } else {
      ...
      if (existingUrl && existingUrl.includes('/uploads/')) {
        await this.deleteFromLocal(existingUrl);
      }
    }
